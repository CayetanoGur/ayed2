...
//FALTA AGREGAR INVARIANTES EN LAS FUNCIONES

static bool invrep(graph_t g) {
    bool res = (g != NULL) && ((g->max_vertexs == 0 && g->costs == NULL) ||
...
void graph_add_edge(graph_t graph, vertex_t from, vertex_t to, cost_t cost)
{
    ...
    unsigned int index = g_index(from,to,graph->max_vertexs); //esto es porque lo estamos representando como un arreglo
