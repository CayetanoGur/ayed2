...
int main(int argc, char *argv[]) {
    ...
    printf("\nLa minima temperatura historica es: %d \n", min_temp(array));

    //max temp for each year

    int output[YEARS];
    procedimiento(array, output);

    for(unsigned int i = 0; i < YEARS; ++i){
        printf("The max temperature for %u is: %d\n", 1980+i, output[i]);
    }

    //rainfall

    int res[YEARS][MONTHS];
    proc_rainfall(array, res);
    
    for (unsigned int i = 0; i < YEARS; ++i){

        for (unsigned int j = 0; j < MONTHS; ++j){

            printf("Year:%d Month:%d Rainfall:%d \n",i+1980,j+1,res[i][j]);
        }
    }
