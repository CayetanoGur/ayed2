...
//No hace falta inicializar new->next = NULL, ya que calloc inicializa los elementos en 0, y 0 es lo mismo que NULL para un puntero.
...
set set_clone(set s) {
    ...
    set node = s;
    while(node != NULL){
        clone = set_add(clone, node->elem);
        node = node->next;
    }
    assert((set_cardinal(clone) == set_cardinal(s)) && (clone != s || set_is_empty(s)));
    return clone;
}
