...
static unsigned int process_unsigned(FILE *file) {
    ...
    int res = fscanf(file, " %d ", &scan_value);
    value = scan_value;
    if (res != 1u || scan_value < 0) {
...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        k_year = process_unsigned(file);
        if(k_year < FST_YEAR || k_year > LST_YEAR){
            fprintf(stderr, "Invalid array.\n");
            exit(EXIT_FAILURE);
        }
        k_month = process_unsigned(file);
        if(k_month > 12){
            fprintf(stderr, "Invalid array.\n");
            exit(EXIT_FAILURE);
        }
        k_day = process_unsigned(file);
        if(k_day < FST_DAY || k_day > 28){
            fprintf(stderr, "Invalid array.\n");
            exit(EXIT_FAILURE);
        }
