...
bool is_match_correct(war_match match) {
  /*tengo un problema aca estoy viendo mal si la siguiente carta es correcta o no y me da false cuando deveria dar true
  mi problema es que cuando paso al siguiente nodo tengo que ver si es null o no porque hacer card_player(aux->card) de un nodo null 
  da segmentation fault, a causa de eso el winner points tambien da mal, la verdad que entiendo que esta pasando y porque da mal pero
  pude terminar de resolverlo */
  if(match != NULL){
    bool start_fst = card_player(match->card) == 1;
    bool size_even = match_size(match) % 2 == 0;
    bool fst_snd = true;
    war_match aux = match;
    while(aux != NULL && fst_snd){
    ...
    return fst_snd && start_fst && size_even;
  } else{
    return true;
  }
}
